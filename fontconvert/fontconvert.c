...
#include "../.pio/libdeps/ESP-32S/Adafruit GFX Library_ID13/gfxfont.h" // Adafruit_GFX font structures
...
FT_ULong sjisToUnicode(char sjis) {
  unsigned long sjisl = ((unsigned long)sjis) & 0xFF;
  if (sjisl < 0x80) {
    return sjisl;
  }
  if (sjisl <= 0x9F) {
    // 0xC0～0xDFは認識されないので、0x80～0x9Fにマッピングする
    return sjisl + 0x40 + 0xFEC0;
  }
  if (sjisl < 0xA1) {
    return sjisl;
  }
...
int main(int argc, char *argv[]) {
  int i, j, err, size, first = ' ', last = 0xBF, bitmapOffset = 0, x, y, byte;
  char *fontName, c, *ptr;
  FT_Library library;
  FT_Face face;
  FT_Glyph glyph;
  ...
  for (i = first, j = 0; i <= last; i++, j++) {
    ...
    if ((err = FT_Load_Char(face, sjisToUnicode(i), FT_LOAD_TARGET_MONO))) {
